...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        #initialise with a empty dictionary
        self.qvalues = {}
    ...
    def getQValue(self, state, action):
        ...
        if (state, action) in self.qvalues:
            ...
            #not seen this state/ Q node value
            return 0.0
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            immed = self.getQValue(state, action)
            qvalues.append(immed)
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            if self.getQValue(state, action) == best_value:
                optimalMove.append(action)
    
        if len(optimalMove) == 0: 
            return None
        else: 
            return random.choice(optimalMove)
